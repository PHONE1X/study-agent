...
Правила: тезисов 10-16, вопросов 8-12 — они должны проверять понимание, а не память на слова. В terms клади только то, что реально встречалось. В unclear клади только то, что действительно выглядит как ошибка распознавания речи; если такого нет, оставь пустой список.

КАЖДЫЙ ТЕЗИС ДОЛЖЕН ЧЕМУ-ТО УЧИТЬ. Тезис — это то, чего человек не знал до занятия: конкретный порядок действий, число, различие между двумя вещами, причина, название инструмента и его назначение. Общие слова вроде «важно донести ценность», «нужно избегать лишних деталей», «клиенту важна понятная информация» — это не тезисы, их можно написать про любое занятие на свете, не слушая его. Если тезис остаётся верным и после подстановки другой темы — выбрось его и возьми вместо него что-то конкретное из заметок.
...
begin
    ...
    # Разделов «Смотреть в записи» и «Обращения к тебе» здесь больше нет.
    # Оба убраны по прямой просьбе: конспект читают ради материала, а не ради
    # напоминаний о том, кто к тебе обращался и что нужно досмотреть глазами.
    # Данные никуда не делись -- обращения лежат в [[Обращения]] и попадают
    # туда сразу, а отсылки к экрану остаются в транскрипте с таймкодами.
    # Конспект теперь состоит только из того, что реально учат: тезисы,
    # вопросы для самопроверки, термины и числа.
